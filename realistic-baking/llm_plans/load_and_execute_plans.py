...
r = range(0, 22) if test else range(0,4)
if test:
    problems = {idx: [f'{t}/problem{idx + 1}.txt'] for idx in r}
else:
    dirs = [(idx, f'{t}/{idx+1}') for idx in r]
    problems = {idx: [] for idx in r} 
    for idx, d in dirs:
        for fname in os.listdir(d):
            problems[idx].append(f'{d}/{fname}')

for idx, fnames in problems.items():
    print(f"*************************PROBLEM {idx + 1}*************************")
    for fname_i, fname in enumerate(fnames):
        print(f"*************************PlAN {fname_i}*************************")
