...
class LLMAgent:
    ...
    def give_plan_failure_feedback(self):
        prompt = f"""Make sure that you are combining the right ingredients in the right way, making sure that the entire plan makes sense."""
    ...
    def get_action(self, obs, problem_idx, planning_attempts):
        ...
        if len(self.action_name_sequence) == 0:
            self.action_name_sequence, self.instruction_steps, self.plan_sketch_conversation  = self._query_LLM_for_action_name_sequence(obs, problem_idx, planning_attempts)
            self.action_number = 0
        action_name = self.action_name_sequence.pop(0)
        instruction = self.instruction_steps.pop(0)
        action = self._query_LLM_for_action(obs, problem_idx, planning_attempts, action_name, instruction, self.plan_sketch_conversation, self.action_number)
    ...
    def _query_LLM_for_action_name_sequence(self, obs, problem_idx, planning_attempts):
    ...
    def _query_LLM_for_action(self, obs, problem_idx, planning_attempts, action_name, instruction, conversation, action_i):
        with open('predicate_and_goal_descriptions.json', 'r') as f:
            descriptions = json.load(f)
        initial_state = ''
        for lit in obs.literals:
